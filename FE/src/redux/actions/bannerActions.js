...
export const createBanner = (bannerData) => async (dispatch) => {
      try {
            ...
            const { data } = await axios.post("/api/v1/banners/admin/banner", bannerData, config);
...
export const getBannerDetails = (id) => async (dispatch) => {
      try {
            ...
            const { data } = await axios.get(`/api/v1/banners/admin/banner/${id}`);
...
export const deleteBanner = (id) => async (dispatch) => {
      try {
            ...
            const { data } = await axios.delete(`/api/v1/banners/admin/banner/${id}`);
...
export const updateBanner = (id, bannerData) => async (dispatch) => {
      try {
            ...
            const config = { headers: { "Content-Type": "multipart/form-data" } };
            const { data } = await axios.put(`/api/v1/banners/admin/banner/${id}`, bannerData, config);
